...
| --- | --- | --- | --- |
| [Chrome WebMCP + Model Context Tool Inspector](https://developer.chrome.com/docs/ai/webmcp) | Chrome’s proposed standard and live page-tool inspector: registered tools, manual calls, schema checks, structured results/errors. | Live inspection/debugging, not documented canonical baseline drift classification, replay-time inventory rejection, seeded UI/tool schedule exploration, fault injection, minimisation, or portable replay. | Complement: develop/debug tools in Chrome; pin and compare observed contracts, then run resilience scenarios in local/CI environments. |
...
5. **Contract drift is a replay guard, not just a report.** Canonical redacted fingerprints cover tool names, input/output schemas, and annotations. Baseline comparisons classify structural changes separately from behavioural drift, while optional scenario assertions pin required inputs, safety annotations, versions, result codes, and deterministic result counts. This detects declared structural and observable drift; it does not infer unchanged business meaning. [tool contracts](../src/webmcp_resilience/tool_contracts.py), [scenario model](../src/webmcp_resilience/models/scenario.py)
...
- “Automatic semantic equivalence.” Structural fingerprints and explicit observable assertions cannot prove that implementation or business meaning stayed the same.
