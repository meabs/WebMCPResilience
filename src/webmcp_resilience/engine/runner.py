...
class ScenarioRunner:
    def __init__(self, page: Page, scenario: Scenario, state_script: str | None = None, from_origins: list[str] | None = None, allow_mutations: bool = False, *, base_url: str | None = None) -> None:
        self.page, self.scenario = page, scenario
        self.adapter = WebMCPAdapter(page, state_script, from_origins)
    ...
    async def _ensure_adapter(self) -> None:
        if self._adapter_stale:
            ...
            self.tools = {tool["name"]: tool for tool in await self.adapter.get_tools()}
            self._adapter_stale = False
    ...
    async def run(self, schedule: list[ScheduledAction] | None = None) -> TraceRun:
        ...
        for tool in await self.adapter.wait_for_tools(required):
    ...
    async def _execute(
        ...
        *,
        ...
        if action.invoke or action.retry:
            ...
            if prestart_tool:
                await self.adapter.start_tool(name, arguments, invocation_id)
                if deferred is None:
        ...
        *,
        ...
        try:
            ...
            if prestarted:
                ...
                result = await asyncio.wait_for(self.adapter.invoke_tool(name, args, invocation_id), timeout_ms / 1000)
            else:
                result = await self.adapter.invoke_tool(name, args, invocation_id)
